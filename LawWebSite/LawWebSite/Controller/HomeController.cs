...
namespace LawWebSite.Controller
{
    ...
    {
        /// <summary>
        /// Veritabanındaki tüm home listeleyen metod
        /// </summary>
        /// <returns></returns>
        public ReturnModel<Models.Home> GetHomes()
        {
            ReturnModel<Models.Home> returnModel = new ReturnModel<Models.Home>();
            ...
            try
            {
                using (DBLAW23Entities ent = new DBLAW23Entities())
                {
                    ...
                    var getHomes = ent.Homes.Include("Language").ToList();
                    if (getHomes != null && getHomes.Count > 0)
                    {
                        returnModel.Is_Error = false;
                        returnModel.Message_Header = string.Empty;
                        returnModel.Message_Content = string.Empty;
                        returnModel.Model = getHomes;
                        goto ReturnPointer;
                    }
                    else
                    {
        ...
        /// <summary>
        /// Sistemde seçili olan dile ait home listeleyen metod
        /// </summary>
        /// <param name="LanguageId">Dil Id Bilgisi Parametresi</param>
        /// <returns></returns>
        public ReturnModel<Models.Home> GetHomesByLanguageId(int LanguageId)
        {
            ReturnModel<Models.Home> returnModel = new ReturnModel<Models.Home>();
            ...
            try
            {
                using (DBLAW23Entities ent = new DBLAW23Entities())
                {
                    ...
                    var getHomes = ent.Homes.Include("Language").Where(x => x.LanguageId == LanguageId).ToList();
                    if (getHomes != null && getHomes.Count > 0)
                    {
                        returnModel.Is_Error = false;
                        returnModel.Message_Header = string.Empty;
